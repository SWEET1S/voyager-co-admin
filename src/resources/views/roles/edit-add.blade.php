                    ...
                    <form class="form-edit-add" role="form"
                        ...
                        <div class="panel-body">
                            ...
                            <a href="#" class="permission-select-all">{{ __('voyager::generic.select_all') }}</a> / <a
                                href="#" class="permission-deselect-all">{{ __('voyager::generic.deselect_all') }}</a>
                            <ul class="permissions checkbox">
                                ...
                                @foreach(Voyager::model('Permission')->all()->groupBy('table_name') as $table => $permission)
                                    ...
                                    @if(config('voyager-co-admin.running') && $table != "")
                                        @php
                                            $hasPermissionInTable = false;
                                        @endphp

                                        @foreach($currentPermissions as $currentPermission)
                                            @php
                                                if(str_contains($currentPermission, $table)){
                                                        $hasPermissionInTable = true;
                                                }
                                            @endphp
                                        @endforeach

                                        @if(!$hasPermissionInTable)
                                            @continue
                                        @endif
                                    @endif
                                    ...
                                    <li>
                                        ...
                                        <ul>
                                            @foreach($permission as $key => $perm)
                                                ...
                                                @if(!in_array($perm->key, $currentPermissions) && config('voyager-co-admin.running'))
                                                    @continue
                                                @endif

                                                <li>
                                                    <input type="checkbox" id="permission-{{$perm->id}}"
                                                           name="permissions[{{$perm->id}}]" class="the-permission"
                                                           value="{{$perm->id}}"
